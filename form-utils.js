...
function initFloatFields() {
  document.querySelectorAll('input[float]:not([data_float_initialized])').forEach(el => {
    ...
    el.setAttribute('data_float_initialized', 'true');
  });
}
...
function initFilePreview() {
  document.querySelectorAll('[file_preview]').forEach(el => {
    const inputName = el.getAttribute('file_preview');
    ...
    input.addEventListener('change', () => {
      ...
      [...input.files].forEach(file => {
        ...
        wrap.setAttribute('data_file', file.name);
...
function initCopyInput() {
  document.querySelectorAll('[copy_input]').forEach(el => {
    const inputName = el.getAttribute('copy_input');
    const updateText = el.getAttribute('copy_input_update');
...
function initStrength() {
  document.querySelectorAll('[strength]').forEach(el => {
    ...
    const messages = (el.getAttribute('strength_messages') || 'Weak;Fair;Strong;Very strong').split(';');
    ...
    input.addEventListener('input', () => {
      ...
      if (val.length === 0) {
        el.textContent = '';
        el.removeAttribute('data_strength');
        return;
      }
      ...
      el.setAttribute('data_strength', score + 1);
    });
  });
}

function initShowPassword() {
  document.querySelectorAll('[show_password]').forEach(el => {
    const inputName = el.getAttribute('show_password');
    const input = document.querySelector(`[name="${inputName}"], #${inputName}`);
    if (!input) return;
...
function initCountdown() {
  document.querySelectorAll('[countdown]').forEach(el => {
    ...
    const labelDays = el.getAttribute('countdown_days') || 'days';
    const labelHours = el.getAttribute('countdown_hours') || 'hours';
    const labelMinutes = el.getAttribute('countdown_minutes') || 'minutes';
    const labelSeconds = el.getAttribute('countdown_seconds') || 'seconds';

    const tick = () => {
      const diff = target - Date.now();

      if (diff <= 0) {
        el.textContent = el.getAttribute('countdown_done') || '00:00:00:00';
        return;
      }
...
function initClearInput() {
  document.querySelectorAll('[clear_input]').forEach(btn => {
    const inputName = btn.getAttribute('clear_input');
    const input = document.querySelector(`[name="${inputName}"], #${inputName}`);
    if (!input) return;
...
function initCountChar() {
  document.querySelectorAll('[count_char]').forEach(el => {
    const inputName = el.getAttribute('count_char');
    const max = el.getAttribute('count_char_max');
    ...
    const update = () => {
      ...
      if (max) el.setAttribute('data_over', len > parseInt(max));
    };
...
function initDirtyWarning() {
  document.querySelectorAll('form[dirty_warning]').forEach(form => {
    const message =
      form.getAttribute('dirty_warning') ||
...
function initOnlyLetters() {
  document.querySelectorAll('input[only_letters]').forEach(input => {
...
function initOnlyNumbers() {
  document.querySelectorAll('input[only_numbers]').forEach(input => {
    input.addEventListener('keypress', e => {
...
function error(name, message) {
  ...
  const existingMsg = input.parentElement.querySelector(`[data_error_for="${name}"]`);
  if (existingMsg) existingMsg.remove();

  const msg = document.createElement('div');
  msg.setAttribute('data_error_for', name);
  msg.setAttribute('role', 'alert');

  msg.style.cssText =
    'display:flex;align-items:center;gap:0.3rem;color:#e53e3e;font-size:0.82rem;margin-top:0.35rem;font-family:sans-serif;';
